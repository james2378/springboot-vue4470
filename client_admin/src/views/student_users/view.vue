			...
			<el-col :xs="24" :sm="12" :lg="8" class="el_form_item_warp">
				...
				<el-form-item label="学生性别" prop="student_gender">
					...
					<div v-else-if="$check_field('get','student_gender')">{{form_sub['student_gender']}}</div>
								...
								<el-col v-if="user_group === '管理员' || $check_field('get','student_age') || $check_field('add','student_age') || $check_field('set','student_age')" :xs="24" :sm="12" :lg="8" class="el_form_item_warp">
				<el-form-item label="学生年龄" prop="student_age">
												<el-input id="student_age" v-model="form_sub['student_age']" placeholder="请输入学生年龄"
							  v-if="user_group === '管理员' || (form_sub['student_users_id'] && $check_field('set','student_age')) || (!form_sub['student_users_id'] && $check_field('add','student_age'))"></el-input>
